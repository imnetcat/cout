...
namespace Core
{
	namespace Filesystem
	{
		...
		public:
			...
			{
				if (!exist())
					throw Exceptions::Core::file_not_exist("source file not found when moving");
				if (!dest.exist())
					throw Exceptions::Core::dir_not_exist("destination not found when moving");
				if (dest.listing().contains((*this)))
					throw Exceptions::Core::file_already_exist("moving destination already contains source file");

				bool test_o_access = std::ofstream(_path, std::ios::app).good();
				if (!test_o_access)
					throw Exceptions::Core::non_readable("moving file descryptor busy");
				bool test_i_access = std::ifstream(_path).good();
				if (!test_i_access)
					throw Exceptions::Core::non_writable("moving file descryptor busy");
